...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const now=Date.now();
    const trial=e?.trial_started_at?new Date(e.trial_started_at).getTime()+30*86400000:0;
    const paid=e?.premium_until?new Date(e.premium_until).getTime():0;
    if(!(trial>now||paid>now))return json({premium_required:true},402);
...
La propriété reply doit pouvoir être affichée telle quelle.`;
    ...
    if(!parsed || typeof parsed?.reply!=="string" || !parsed.reply.trim()){
      return json({error:"empty_ai"},502);
    }
